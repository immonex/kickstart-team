...
class Turnstile {
	...
	public static function verify( $token ) {
		...
		$utils        = apply_filters( 'inxkicktm_get_utils', false );
	...
	public static function add_consent_note( &$consent_text ) {
		...
		if ( ! $turnstile_note ) {
			$turnstile_note = wp_sprintf(
				...
				__( 'By confirming, <a href="%s" target="_blank">Cloudflare Turnstile</a> will be activated, which we use to protect our forms from spam and bots.', 'immonex-kickstart-team' ),
				__( 'https://www.cloudflare.com/privacypolicy/', 'immonex-kickstart-team' )
			);
		}
